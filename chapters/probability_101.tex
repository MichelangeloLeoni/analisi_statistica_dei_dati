...
\begin{example}
	Consider an observable $x\in \mathbb{R}$ that follows a \emph{pdf} $p_x(x)$. Consider now a statistic $y=x^2$. We want to find $p_y(y)$.
	
	Clearly $x(y)=\pm \sqrt{y}$, then we have two part of the original \textit{pdf} mapped to the newer (see \ref{fig:variable_change}), thus we have to sum the probabilities. 
