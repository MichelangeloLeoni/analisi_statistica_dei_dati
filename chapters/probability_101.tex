...
\begin{example}[Demonstration of the LLN]
	The most general demonstration of the LLN is beyond the scope of this course and therefore this write-up, however we will look at a more limiting but still instructive example. Let's suppose that also the following integral exists (\textit{variance of} $X$):
	\[
...
\begin{lemma}[Čebyšëv's theorem]
	...
	\label{eq:chebyshev}
	\end{equation}
	exists for each $k\in\R^+$. While it's not the most useful upper bound it's useful in the demonstration of the LLN. 
\end{lemma}

\begin{example}[Demonstration of Čebyšëv's theorem]
		\begin{align*}
...
Now we can get to demonstrating the LLN, armed with the Čebyšëv inequality. We notice that the \textit{pdf} of the vectorial observable $\{x_i\}_N$ can be written as the product of the \textit{pdf's} of the single observables, assuming that the observations are all independent: $p(x_N)=\prod_i^N p_X(x_i)$. Therefore, we have that
\begin{align*}
