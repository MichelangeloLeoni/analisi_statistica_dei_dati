...
\begin{definition}[Covariance]
	...
	\]
\end{definition}
We get that the covariance is null for independent variables, but not vice-versa ($f=x$, $g=x^2$)\todo{If we are feeling less lazy than I am now, we could show this proof in an example}
...
\begin{theorem}
	If $p_1(x)$ and $p_2(x)$ have the same moments and $\left(p_1(x)-p_2(x)\right)$ is expandable in a power series then 
	\[
	p_1(x) = p_2(x)
	\]
\end{theorem}
...
\begin{align*}
	f(\vec{x}) &\simeq f(\vec{\mu}) + \frac{\partial f}{\partial x_i}(x_i-\mu_i)+\frac{1}{2}\frac{\partial^2f}{\partial x_i \partial x_j}(x_i-\mu_i)(x_j-\mu_j)+\dots \\
	&\Rightarrow \E[f(\vec{x})] \simeq f(\vec{\mu}) \frac{1}{2}\frac{\partial^2f}{\partial x_i \partial x_j}\operatorname{cov}[x_i,x_j]
\end{align*}

It's pretty easy to convince ourselves that
	\[
	\operatorname{cov}[f_1,f_2] = \sum_{i,j}\frac{\partial f_1}{\partial x_i}\frac{\partial f_2}{\partial x_j}\operatorname{cov}[x_1,x_j]+\dots
	\]
Which for uncorrelated $x$ (diagonal covariance matrix) simplifies to:
	\[
	\sigma_f^2 = \Var[f] = \sum_i \left| \frac{\partial f}{\partial x_i} \right|^2\sigma_i^2\sum \dots
	\]
	
We recognize this as our familiar \textbf{formula for the propagation of errors}. Interesting to note that this formula comes from a Taylor approximation.\todo{This whole sequence isn't totally clear to me, so I'd like to learn it better and try to then explain it better, as I simply copied from the slides. Also, is this the right place to put this?}
...
\begin{example}[Proof of the central limit theorem]
	...
	\]
	We note that already we have $\Var[y_n]=\sigma^2$ independently of $n$, as the variance is additive and goes with the square of any multiplicative constant. Using the properties of the characteristic function we have just seen, we define:
	\[
