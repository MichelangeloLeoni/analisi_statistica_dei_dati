...
\begin{example}
	A good way to demonstrate the concepts we've encountered until now is to solve the famous \textbf{Monty Hall Problem} (You've \emph{probably} heard about it at least one in your life).
	
	Image you are trasported from the desk where you are reading this to the studios of a famous game show. The conductor, Mr. Monty, explain you the rules of the game: the are three closed doors behind you. One conteins a brand new sports car, the other two a goat. You choose one door to open, you win the prize behind it. Obviously Mr. Monty knows behind which door is hidden the car.
	
	You have read what comes before this example, so you know that every door has the same probability of $\frac{1}{3}$ of conteining the car. So you choose, say, the first door (it doesn't really matter which door you choose for what comes).
	
	Now Monty want to spice up things for the audience watching and, after you choose door one, opens door three revealing a goat behind it (to be clear: Mr. Monty cannot open the door you choose, in this case door number one, and cannot open the door hiding the car, the host revealing the car location is not a great spectacle, isn't it?).
	Now he want you to make a decision: do you wish to stick with the choice of door number one or do you change your choice and switch to door number two?
	This seems just a mind game, ultimately you have the same one third of chance to get the car anyways. But the reality, as usual when working with probability, is far more complex and could seem almost paradoxical. So you should take a moment to think, take out of pocket your hand sketchbook in front of the crowd and do some math.
	
	We can tackle the problem highlighting six different events:
	\begin{itemize}
		\item $C_i$, the car is behind door number $i$
		\item $A_i$, Mr. Monty opens door $i$ revealing a goat
	\end{itemize}
	with $i \in (1,2,3)$.
	To have the best chance of winning the car you want to find:
	\[
		P(C_1|A_3)\lesseqqgtr P(C_2|A_3)
	\]
	in other words: if Monty opened the third door revealing a goat, you have better chance sticking with door one or switching to door two?
	
	As you correctly first guessed the probability of finding the car behind door $i$ is:
	\[
		P(C_i)=\frac{1}{3}
	\]
	We can make use of the Bayes' theorem \ref{th:Bayes} to find the probability of the car being behind door number one given Mr. Monty opened door number three:
	\[
		P(C_1|A_3)=\frac{P(A_3|C_1)P(C_1)}{P(A_3)}
	\]
	now we only have to compute each term.
	We already know that $P(C_1)=\frac{1}{3}$. The first term of the numerator represent the probability of Mr. Monty opening the third door given the car is behind the first door. Now, since he knows that we pick the right one, he can only open doors two or three, each hiding a goat. Thus the probability that he will opening the second or third door is the same:
	\[
		P(A_3|C_1)=\frac{1}{2}
	\]
	Now we need to compute the overall probability of Mr. Monty opening the third door, revealing a goat, $A_3$. We can use the total probability law \ref{th:total_prob_law}:
	\begin{align*}
		P(A_3)&=\sum_i P(A_3|C_i)P(C_i)\\
		&=P(A_3|C_1)P(C_1)+ P(A_3|C_2)P(C_2)+ P(A_3|C_3)P(C_3)\\
		&=\frac{1}{3}(P(A_3|C_1)+P(A_3|C_2)+P(A_3|C_3))
	\end{align*}
	We already know the value of the first term. The second term represent the probability of Mr. Monty opening the third door given the car is behind the second door. Since he cannot open the first door, that we choose, and the second door, that hide the car, his only choice is to open the third. Thus the second term is equal to 1. The third therm represent the probability of Mr. Monty opening the third door giveng the car is behind the door itself. We know that he cannot open the door conceiling the car, thus the third term is equal to 0. Substituiting the correct values, we get:
	\[
		P(A_3)=\frac{1}{3}\left(\frac{1}{2}+1\right)=\frac{1}{2}
	\]
	Thus:
	\[
		P(C_1|A_3)=\frac{\frac{1}{2}\frac{1}{3}}{\frac{1}{2}}=\frac{1}{3}
	\]
	Leaving us with:
	\[
		P(C_2|A_3)=1-P(C_1|A_3)=\frac{2}{3}
	\]
	This means that switching door and choosing door number two doubles your probability of winning the car!
	This is huge, and seems too good to be true. The results comes from the fact that Mr. Monty doesn't behave casually, because he knows where the car is hidden, and it's choice is conditioned by this knowledge. Him revealing the position of one of the goat changes our posterior probability.
	So you should put back the sketchbook and switch door, every time, and come back to where you were studying riding your new sport car (at least probably :) ).
\end{example}

The last expression takes the name of \textbf{Bayes' theorem}. Interestingly enough, it is not what characterizes Bayesian statistics, and we remind that we are still in the Kolmogorov frequentist approach, though the theorem generalizes to the Bayesian approach if the probabilities are replaced with degrees of belief, which we will briefly introduce in this next section.
...
\begin{example}[Proof of the LLN]
	The most general proof of the LLN is beyond the scope of this course and therefore this write-up, however we will look at a more limiting but still instructive example. Let's suppose that also the following integral exists (\textit{variance of} $X$):
	\[
...
Now we can get to proving the LLN, armed with the Čebyšëv inequality. We notice that the \textit{pdf} of the vectorial observable $\{x_i\}_N$ can be written as the product of the \textit{pdf's} of the single observables, assuming that the observations are all independent: $p(x_N)=\prod_i^N p_X(x_i)$. 
...
If that didn't satisfy us the reader will be pleased to know that there is a stronger version of the Law of Large Numbers, appropriately called "Strong-LLN". 
...
We have, in the previous sections, talked about operators such as the distribution, expected value, and variance of random variables. It is now time to generalize these definitions to statistics, which we remind are \textit{functions} of the random variables themselves. 

\subsubsection{Probability distribution of a statistic}
Let's begin with the most pressing one: how do we define the \textit{distribution} of a \textit{statistic}? Well, if I know the discrete \textit{pmf} of $x$ then the \textit{pmf} of the statistic $f(x)$ is easily written as $pmf\left(x(f)\right)$, no surprises there. 
...
\begin{example}
	Suppose we start from a known distribution in $x$, and want to find the \textit{function} $f$ that brings us to a known distribution $p_f(f)$. The, inverting \ref{def:pdf_var_change} we arrive at:
	\[
...
\begin{example}
	...
	For simplicity, we'll assume that the velocity of the gas follows the following distribution (which we will later learn to be the very important \textit{uniform distribution})
	\[
...
\begin{definition}[$n$\textit{-th} order moment of a distribution]
	\[
...
\begin{definition}[Moment generating function]
	We define \textit{moment generating function} for a distribution 
	\[
	M(t) = \E[e^{tx}]
	\]
...
$M(t)$ has the nice property that for $x=x_1+x_2$ (with $x_1,x_2$ \textit{independent}) $M(t)=M_1(t)M_2(t)$. We see this right away if we explicitly write 
	\[
...
The integrand is a holomorphic function with no poles\sidenote{whatever that means} thus we can perform the substitution $z=\sigma t -ix/\sigma$:
...
This is the Gaussian (or normal) \textit{pdf}, and the way we derived it highlights its fundamental importance in statistics. It is important to emphasize that the shape of this limit distribution does not depend on the original distribution of $x$.\\
