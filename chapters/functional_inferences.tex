...
Things start to get interesting when the distribution itself depends on some unknown parameter\sidenote{This parameter can be anything, it does not have to be observable, follow a \textit{pdf}, be a random variable, it can be anything that the density function of the observable depends on.}, and measuring the observable brings us new information on what that parameter may be. In this section we will write
	...
	p_X(x;\theta)
	\]
to denote the \textit{pdf} of observable $x\in X$ dependent on some parameter $\theta\in M$, using the semicolon to distinguish observables from parameters. Our goal is that of the \textit{functional inference}, finding a \textit{function} that depends on the observables (statistic) that assigns a sort of "score" to every value of the parameters, indicating their plausibility. In short:
	\[
...
The likelihood is not a probability nor probability density, but in general a non-negative function that indicates the "plausibility" (well, \textit{likelihood}) of extracting whichever $\theta$ given the measure $x_0$.
...
\begin{enumerate}
	...
	\item When transforming variables, the Likelihood transforms like the \textit{pdf} (so with $||J(x)||$), and therefore the ratio $\frac{L\left(\theta_1\right)}{L\left(\theta_2\right)}$ of two Likelihoods remains constant.
	...
	\item If we apply an invertible transformation in the 	\textit{parameter} space, the Likelihood changes functional form but maintains its value ($L_x(f(\theta)) = L_x(\theta)$). 
	\begin{example}
...
and using Bayes' theorem (\ref{th:Bayes}) we can write:
	\begin{equation}
	p(\theta|x_0) = \frac{\overbrace{p(x_0|\theta)}^{L_{x_0}(\theta)}p(\theta)}{p(x_0)}
	\label{def:posterior}
	\end{equation}
 
 In words: the \textbf{posterior} probability $p(\theta|x_0)$ is the product of the Likelihood times the \textbf{prior} probability $p(\theta)$ (times a multiplicative normalization factor $p(x_0$)). When it exists, it is naturally a better tool than the likelihood function for functional inferences as it builds upon it by adding the information tied to the distribution of $\theta$ itself. 
 ...
 Often times, to avoid calculating the normalization, we talk about the ratio
 	...
 	\]
 where $LR$ (\textit{Bayes' factor}) "updates" the relative probability between two hypotheses (different $\theta$) at each observation. Given the multiplicative nature of the Likelihood, this formula can be used to track the "evolution" of the relative probability. 
...
Let's now talk about the more realistic scenario where $\theta$ does not admit a probability function (or it does, but we don't know it). The Bayesian (subjectivist) approach proceeds by \textit{assigning} a prior distribution to $\theta$, based on the \textit{degree of belief} $\pi(\theta)$. Then, the calculations proceed as before, and by treating the degree of belief as a probability we arrive to the posterior degree of belief:
	\[
...
Once again, we can talk about credibility ratios
	...
	\]
 and the quantity $\frac{L_{x_0}(\theta_1)}{L_{x_0}(\theta_2)}$ is called \textit{belief-updating ratio} while $\frac{\pi(\theta_1)}{\pi(\theta_2)}$ hold the significance of \textit{betting odds} in the game theory world.
...
\begin{definition}[Sufficient statistic]
	Let $p(x;\theta)$ be the \textit{pdf} of $x$, depending on parameter $\theta$. Let $s(x)$ be a statistic (with dimensionality less than or equal to that of $x$) such that the probability of $x$ \textit{given} $s$ does not depend on $\theta$:
	\[
	p(x|s;\theta_1) = p(x|s;\theta_2) \forall\theta_1,\theta_2
	\]
	If this is the case, we say that the statistic $s$ is \textit{sufficient for} $\theta$.
\end{definition}
\begin{corollary}
...
\begin{definition}[Ancillary statistic]
	The less used complementary cousin of the sufficient statistic: if $s(x)$ is a statistic such that its \textit{pdf} is not dependent on $\theta$:
	\[
	p(s;\theta_1) = p(s;\theta_2) \, \forall \theta_1,\theta_2
	\]
	Then we say that the statistic is \textit{ancillary} for $\theta$
\end{definition}
\begin{theorem}[Factorization theorem]
	...
	\]
	Important note: the domain of $x$ must be \textit{independent of} $\theta$ (see \ref{ex:uniform_sufficient_statistics})
\end{theorem}
...
To put everything together, let's extend the concept of Fisher Information to \textit{statistics}, given a statistic $s(x)$ let's call the \textit{Information} of $s$ on $\theta$ the quantity:
	\[
