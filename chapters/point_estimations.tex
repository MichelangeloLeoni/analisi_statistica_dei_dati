...
Let's verify the conditions for the existence of at least one consistent estimator through the solution of the above equation. We need to check that the expected value of its derivative exists not null, as described in definition (\ref{def:implicit_estimator}).\todo{This came out of my brain hole it's not written anywhere but I believe it's right. Delete this if you agree.}
\begin{align*}
...
This expression is different from zero if $V$ is positively-defined, in which case at least one consistent estimator for $\theta$ exists. \todo{We never checked the expected value but ok...}
