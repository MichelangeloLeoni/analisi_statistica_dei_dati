...
we usually intend 
	\[
	\mu = \hat{\mu}(x_0) \pm \sqrt{\Var\left[\hat\mu\right]}\bigg\rvert_{\mu=\hat{\mu}}	
	\]
where we explicitly write that $\hat{\mu}$ depends on the observations $x_0$ and that $\Var\left[\hat\mu\right]$ is a function of $\mu$ evaluated at $\hat{\mu}$.\todo{There are some questions left as exercises here if we want to include them, it's Saturday and I don't feel like thinking about them right now...}
...
Now that we understand the power of the MLE, we see why it is so widely used in real-world problems. In more complex cases, especially with large datasets, the maximum of the total likelihood $L_{\text{tot}}$ is typically found numerically using optimization algorithms (\textit{fitting})\todo{From here we start talking about \textit{fitting} but we never defined what a fit is in the first place}. The question is: how can we evaluate the variance of the MLE in these cases?\todo{Is it clear that we are going to take an estimate of the Var because in this cases it is too difficult/impossible to find it analytically?}
...
We can than write:
\begin{align*}
	\ln L (\theta) &\simeq Const + \sum_i k_i \ln (V_i p(x_i;\theta)) \\
	&= Const' + \sum_i k_i \ln p(x_i;\theta)
\end{align*}

In the limit of infinitely fine binning ($n \to \infty$), each bin contains at most one event, so that $k_i \in \{0,1\}$ and:
\begin{align*}
	\ln L(\theta) &= Const + \sum_{i=1}^{N}\ln p(x_i;\theta)\\
	&= Const + \ln L_{unbinned}(\theta) 
\end{align*}
