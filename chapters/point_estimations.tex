...
\[
	N I_1= -N\E\left[\frac{\partial^2\ln L(\mu)}{\partial^2 \mu}\right]=-N\frac{\partial^2}{\partial^2\mu}\E\left[\ln L(\mu)\right]
\]
If the expected value exists, it can be consistently estimated from the data using our loyal LLN:
\begin{align*}
	N I_1=-N\frac{\partial^2}{\partial^2\mu}\E\left[\ln L(\mu)\right]	&\simeq -N \frac{\partial^2}{\partial^2\mu} \frac{1}{N}\sum_{i=1}^N\ln L_i(\mu) \\
	&= \frac{\partial^2\ln L_{tot}(\mu)}{\partial^2\mu}
\end{align*}
Using the fact that $\hat{\mu}$ is a consistent estimator, we finally obtain:
\[
	 \Var(\hat{\mu})\simeq\left(\frac{\partial^2\ln L_{tot}(\hat{\mu})}{\partial^2\mu}\right)^{-1}
\]
...
\]
This result is widely used in fitting software (you've probably used a million times) to evaluate the errors on fitted parameters!
...
\todo{I'll happily skip the hyper-goofy 2D graph present on the slides since we skipped it in the lectures too}