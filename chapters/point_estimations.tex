...
We have seen thus far the basic principles of classic point estimators, where in most cases the \textit{pdf} of the observable is known almost to perfection. However, this is not always the case, and it's important to talk about one final concept: the \textit{robustness} of a statistic. 

\begin{definition}[Robustness of a statistic]
	The \textit{robustness} measures how well a statistic maintains its properties even if the underlying distributional assumptions are incorrect. 
\end{definition}
...
\begin{example}
	A good estimator for the variance of a Gaussian distributed random variable is the \textit{root-mean-square} (RMS), which, being the MLE, is asymptotically efficient:
	\[
	\hat\sigma \equiv s_N = \sqrt{\frac{1}{N}\sum_i (x_i - \bar{x})^2}
	\]
	where $\bar{x}$ is the sample mean.
	
	This is not the only possible estimator for the variance. We could use, for example, the \textit{mean-absolute-deviation} (MAD):
	\[
	...
	Consider the case where we are not completely certain about the underlying \textit{pdf}. For example, suppose the data are contaminated by a small fraction $f$ of observations with a larger variance $\sigma_2$.
	We model the total \textit{pdf} as a mixture of two Gaussians with the same mean $\mu$ but different variances (take $\sigma_2 = 3\sigma_1$):
	\[
	...
	\end{tabular}
	\end{center}
	The result is remarkable: with a contamination of $f=0.05$, $d_N$ becomes about twice as efficient as $s_N$! The theory we have built so far fragile.
...
\begin{itemize}
	\item When $p\rightarrowtail-\infty$ $m$ represents the \textit{mode}, which signifies the single value that appears most frequently. We are minimizing the amount of values that do not equal $m$
	\item When $p=1$ $m$ represents the \textit{median}\sidenote{We will familiarize ourselves with the median in the next section.}, which is the value where half the data lies above and below it. We are minimizing the sum of absolute differences.
...
\begin{definition}[Median]
	The \textit{median} $m_1$ is the value for which there is an equal probability of being greater or less than: $P(x > m_1) = P(x<m_1) = 0.5$. In other words, it is the value for which the cumulative density function is one half. 
\end{definition}
...
The sample median is such an important robust estimator that we also wish to see how it is distributed, since this will be independent of the \textit{pdf} from which the data was extracted.

Let's assume that $p(x)$ is a \textit{pdf} with a singular median, and let's extract $2k+1$ points from it. We can write the the \textit{pdf} of the sample median $\hat{m}_1$ as a multinomial\todo{ref with all the bla bla} where $k$ points are below $\hat{m}_1$ (probability $F\left(\hat{m}_1\right)$), $k$ points are above $\hat{m}_1$ (probability $1-F\left(\hat{m}_1\right)$), and one is exactly $\hat{m}_1$. This expression can be easily written as:
\begin{equation}
	p_1(\hat{m}_1;m_1) = (2k+1)\binom{2k}{k}p(\hat{m}_1)F^k\left(\hat{m}_1\right)\left(1-F\left(\hat{m}_1\right)\right)^k
\label{eq:median_pdf}
\end{equation}

Let's study the asymptotic case of high statistics:
	\[
	\ln p_1\left(\hat{m}_1;m_1\right) = \ln\left(\left(2k+1\right)!\right) - 2\ln\left[F\left(\hat{m}_1\right)\left(1-F\left(\hat{m}_1\right)\right)\right] + \ln p\left(\hat{m}_1\right)
	\]
...
We see that the argument of the logarithm is always less than 1 except for the case in which $\hat{m}_1 = m_1$, and so:

	\[
	\lim_{k \to \infty}  p_1\left(\hat{m}_1;m_1\right) = 0 \forall \hat{m}_1 \neq m_1
	\]
	
And thus the \textit{pdf} of $\hat{m}_1$ converges punctually to $m_1$.

If we go back a few steps to the asymptotic expression of the \textit{pdf}:

	\[
	 p_1\left(\hat{m}_1;m_1\right) \simeq\left(2k+1\right)\exp\left(k\ln\left[4F\left(\hat{m}_1\right)\left(1-F\left(\hat{m}_1\right)\right)\right]\right)
	\]

Expanding the cumulative of $x$ in $\hat{m}_1=m_1+\varepsilon$ we arrive to:

	\[
	F\left(\hat{m}_1\right) \simeq F\left(m_1\right) + p(m_1)\varepsilon = 0.5 + p(m_1)\varepsilon
	\]
	
From which, through substitution:

\begin{align*}
	p_1\left(\hat{m}_1;m_1\right) &\propto \exp\left(k\ln\left[4\left(0.5 + p(m_1)\varepsilon\right)\left(\left(0.5 - p(m_1)\varepsilon\right)\right)\right]\right)\\
	&\simeq \exp\left(k\ln\left(1-4p^2(m_1)\varepsilon^2\right)\right)\simeq \exp\left(-4kp^2(m_1)\varepsilon^2\right)
\end{align*}

Which we can recognize to be asymptotically Normal with variance:

	\[
	\Var\left[\hat{m}_1\right] = \frac{1}{4Np^2(m_1)}
	\]

What this all means is that $\hat{m}_1$ is a consistent estimator for $m_1$ with an asymptotic efficiency greater than 0.
...
Because an unknown \textit{pdf} compromises our statistical certainty, we must adopt robust principles to minimize information loss. This specific form of ambiguity, which stems from a partial understanding of the \textit{pdf}, is what we term \textbf{systematic uncertainty}. Under this definition, we can see robustness as the property of not being sensible to systematic uncertainties. 

\begin{example}
	We are measuring something that has independent counts, and so we assume a Poisson distribution, where the best estimator\sidenotemark for $\mu$ is $\hat\mu = \frac{\sum_i k_i}{N}$ 
\end{example}
\sidenotetext{The calculation for the MLE for the $N$ Poisson extractions is found in \textcolor{orange}{REF}}
