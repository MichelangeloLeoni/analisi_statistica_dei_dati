...
\begin{itemize}
	\item When $p\rightarrow-\infty$ $m$ represents the \textit{mode}, which signifies the single value that appears most frequently. We are minimizing the amount of values that do not equal $m$
...
\begin{definition}[Median]
	The \textit{median} $m_1$ is the value for which there is an equal probability of being greater or less than: $P(x > m_1) = P(x<m_1) = \frac{1}{2}$. In other words, it is the value for which the cumulative density function is one half. 
\end{definition}
...
we can expand the cumulative distribution around $\hat{m}_1 = m_1 + \varepsilon$:
\[
...
Usually, I'll have some information on $b$, like restricting it within a range:
	...
	\[
	b_0 -  \leq \b \leq b_0 + \frac{\delta}{2}
	\]
...
In any case, we say \textit{something} about $b$, and, for example, we can expand $\hat\mu$ to include this:
...
\begin{example}
	Let's look at an interesting case: along with my observable $k$, distributed like a Poisson $Pois(k,\mu+b)$, we have an observable $q$ that has a \textit{pdf} $p(q;b)$. This means that there is \textit{some} observable that gives me information on my background parameter $b$ (from which I was able to determine that $b$ fell within a certain range, for example). 
...
\begin{example}
	Another interesting case is where $b$ has a \textit{pdf} (or \textit{dob} in the Bayesian case) and we can eliminate it by marginalizing:
...
Of course, the result of our measurements will depend on these (and so will the \textit{pdf} of the data), though they are not uncertainties. 
