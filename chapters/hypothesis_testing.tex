...
We frequently use Test statistics of the type $T(x):\C\equiv\{x:T(x)>q_\alpha\}$ with
	
	\[
	\alpha = P(x\in \C;H_0) = \int_{q_\alpha}^{\infty}p(T(x);H_0)\dd T
	\]

of which we take the \textit{sup} for composite hypotheses. We choose a small $\alpha$ beforehand ("significance level") and we use it to find $q_\alpha$, (the \textit{cut-off}). For example, typical values may be $\alpha = 0.05, 3\sigma, 5\sigma,\dots$
...
The final, and most powerful property we can request of our Test is for it to be the \textbf{uniformly most powerful} (UMP), which essentially means that out of all of the Tests it is the "best" one:
	...
	\[
	\forall i: \text{pow}_T(i) \geq \text{pow}_{T^\prime}(i) \forall T^\prime
	\tag{UMP Test}
	\]
...
When we talk about simple hypotheses, it's fundamental to also talk about the \textit{Neyman-Pearson} (NP) Test, and the associated theorem:

\begin{theorem}[Neyman-Pearson]
	Given two simple hypotheses $H_0,H_1$, corresponding to the \textit{pdf's} $p_0(x),p_1(x)$, and let $T_{NP}$ be a Test based on the statistic $T(x) = \frac{p_0}{p_1}$, with critical region $\C\equiv \{x: T(x)<q\}$ and significance level $P_0(\C)=\alpha$.
...
\begin{theorem}
	Let $p(x;\theta)$ be the \textit{pdf} of $x\in\R^N$ dependent on a real-valued parameter $\theta$. We consider the null hypothesis $H_0: \theta= \theta_0$ to be tested against the alternative composite hypothesis $H_\theta : \theta > \theta_0$.
...
In this case, the null hypothesis fixes \textit{some} values of the parameter and leaves some other values as free alternatives. We could think to still use the \textit{LR} statistic as we have done in the UMP Test, as it generalizes to the NP Test\sidenote{Once again, its inverse, though it does not change much} for simple hypotheses, which we know has the property of maximal power. Therefore, we write:

	\[
	\lambda(x) = 2\ln \frac{\sup_{\theta,\nu}p(x;\theta,\nu)}{\sup_{\nu}p(x;\theta_0,\nu)}
	\]

Which we can more intuitively write by substituting the "sup's" with the \textit{MLE} for the specific parameter:

	\[
	\lambda(x) = 2\ln \frac{p(x;\hat{\theta_{\text{MLE}}},\hat{\nu_{\text{MLE}}})}{p(x;\theta_0,\hat{\nu_{\text{MLE}}})}
	\]

This can be seen as a N-P Test done on $H_0$ and the alternative hypotheses obtained by using the \textit{MLE} of the free parameters. We also note that if there exists a sufficient statistic, $\lambda(x)$ depends only on it.
