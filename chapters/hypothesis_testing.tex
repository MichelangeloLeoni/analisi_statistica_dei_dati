...
\todo{LR ratio "nested hypothesis" test can go here?}
...
\begin{theorem}[Neyman-Pearson]
	Given two simple hypotheses $H_0,H_1$, corresponding to the \textit{pdf's} $p_0(x),p_1(x)$, and let $T_{NP}$ be a Test based on the statistic $T(x) = \frac{p_0}{p_1}$, with critical region $\mathcal{C}\equiv \{x: T(x)<q\}$ and significance level $P_0(\mathcal{C})=\alpha$.
	
	Let $T^\prime$ be another Test with critical region $\mathcal{C}^\prime$ and significance $\alpha^\prime\leq\alpha$ 
