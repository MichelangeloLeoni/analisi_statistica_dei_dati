...
\begin{example}
	We've already showed how one can find numerically confidence intervals in the poissonian case. We now want the approximate solution using the LR method.
	
	We know that a sample of $N$ observation $\{x_i\}$ following a Poisson distribution with a parameter $\mu$ the MLE is (\textcolor{red}{ref here the part of the appendix/main chapters where we computed for the first time the MLE of a poisson}):
	\[
