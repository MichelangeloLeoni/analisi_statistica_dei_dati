...
\begin{example}
	...
	The table below reports the likelihood-ratio intervals for observed counts $n = 0, \dots, 9$:
	
	\input{tables/llr_intervals.tex}
	
	It is important to stress that these intervals are only asymptotically valid. In the Poisson case, especially for small values of $n$, the discrete nature of the data leads to deviations from the nominal coverage.
	
	To quantify this effect, one can compute the coverage function as in \ref{eq:coverage}:
	\[
...
\end{example}
\todo{I still have to add another column to the table for the "central limit" intervals, as in the slides. I haven't managed yet to reproduce the number in the slide. I don't understand which method was used to compute those numbers. :(}