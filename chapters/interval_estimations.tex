...
Up until now, several methods for estimating properties of \textit{pdf's} have been discussed along with techniques for obtaining the variance of these estimators. One such method was the point estimate of a parameter, where we discussed, along with the value obtained by the estimate itself, its variance. Essentially, we looked at \textit{how widely} the estimates would be distributed if the experiment would be repeated many times in the same way. What the title of this chapter suggests is that we are now looking for a way to express, up to a certain degree of confidence what interval the parameter that we are trying to estimate falls within. We take a second to note that while it may seem that we have already achieved something similar, this request is fundamentally different than anything our point estimate theory can provide. A naïve reader may say:

	\[
	\hat\theta = a \pm \sigma_a \Rightarrow \hat\theta \in \left[a-\sigma_a, a+\sigma_a\right]
	\]

Though you, as an attentive reader and excellent statistician will immediately identify the crucial flaw in this statement: $\sigma_a$ indicates the \textit{variability} of the estimate in the ensemble of repeated experiments, and does not represent the same "hard" upper/lower bound that we are trying to achieve through interval estimations. Really, the values $a-\sigma_a, a+\sigma_a$ have no more significance than any others in the point estimate formulation. We will learn in this section that if the function that we are sampling is Normal, then the above interval really represents a 68.3\% confidence interval centered around the estimated value.
...
The importance of this scripture cannot be overstated: we are finding an interval in the \textit{parameter space}, and this interval \textit{depends on the observed data}.
...
We can see the power of this Bayesian case, and it's not hard to understand what an interval estimate looks like and means under this formulation. However, it is not always applicable, as the unknown parameters must admit \textit{pdf's} themselves.
...
Namely: \textbf{a function of the parameters at a fixed observation whose positive domain describes an interval in the parameter space}.
If we think about it, on the left side we have an integral in $\dd x$, and on the right a constant that we fix beforehand, namely the "confidence level" \textit{CL}\sidenote{Don't worry about these definitions, they will all get cleared up in the near future. This part aims to provide a general outlook on the math that we will be using, so just bear with us}. Fundamentally, the equation that describes the interval in $\theta$ is, justifiably so, an inequality dependent on the parameter that we must solve. Let's never forget this as it will always be the final step. 

Now, what exactly is this $g_{x_0}(\theta)$? Well, since we are not in the Bayesian case and cannot directly say that the parameter admits a \textit{pdf}, we can only talk about the distribution of the observable: we want to construct an interval such that, over many repetitions of the experiment (different $x_0$), the probability that the resulting interval contains the true $\theta$ is at least \textit{CL}. As we will see, infinite such intervals exist for each fixed $\theta$ (there are infinite ways to integrate a normalized function such that its integral is equal to, say, $0.95$), and $o_{\theta}(x)$ serves as an "ordering function" with the aim of unambiguously defining one such interval by describing which $x$'s to include. 

This integral in eq. \ref{eq:int_definition} is nothing more than the integral of the \textit{pdf} itself, which we know to be the cumulative of the density function\sidenote{\textit{CDF's} start the integration from $-\infty$, so our integral is actually the difference of two \textit{CDF's}}, calculated on upper and lower bounds that may, in general, depend on the parameter. 

What this all means is that having fixed an observed $x_0$ (coming from \textit{pdf} $p(x;\theta)$), the ordering function describes a $\theta$-dependent interval for which the integral of the \textit{pdf} is greater than or equal to the \textit{CL}. Therefore, we can solve this expression (which we so naïvely wrote as $g_{x_0}(\theta) - CL \geq 0$) to find an interval in $\theta$, and that will be the estimate that we are looking for. 

Solving this equation is rarely simple, and can be done when the \textit{pdf} assumes a closed form cumulative, or at least when the latter is tabled\todo{such as the ones in ref.}. The next sections will break down each part of the process, using examples and visual aids to try to better explain this difficult, yet fundamental concept.

\subsection{Confidence levels and coverage}
Let's begin our piece-by-piece breakdown of eq. \ref{eq:int_definition} by being more precise in our definition of \textit{confidence level}:
...
We could start with setting $o_{\theta}(x)=p(x;\theta)$ (\textbf{probability ordering}), which is a universal choice that brings the smallest interval in $x$ (which does not necessarily translate to the smallest interval in $\theta$). This means that we are ordering using the \textit{pdf} itself, choosing the most probable values of $x$ in decreasing order.

Another natural choice would be to use $o_{\theta}(x)=\pm x$, as to start from the lowest value of $x$ and include higher values until the confidence level is reached (essentially finding the value for which the cumulative is equal to \textit{CL}), or vice versa. This is \textbf{lower/upper ordering} (which brings us superior/inferior bounds on the parameter).

We could also think to take the central band (\textbf{central ordering}), as to exclude $\frac{1-CL}{2}$ from the top and bottom. The ordering function $o_{\theta}(x)$ is defined implicitly by the tails of the distribution. We include $x$ values in our acceptance reigon repeating the upper/lower ordering simultaneously on the tails until half of \textit{CL} is reached. 
...
A way to visualize the effect of these different ordering algorithms is through the construction of a "belt" that associates, given the ordering algorithm, each measured value of the observable to an interval estimate. This plot is namely the \textit{Neyman Construction}, is built by solving eq (\ref{eq:int_definition}) for all fixed values of $\theta$. 
...
\label{ex:LR_vs_p_ordering}
	Let's build the Neyman band for a Normally distributed $x$ with unknown parameter $\mu$ (let's say $\sigma=1$ for simplicity). We choose, for the sake of this example, to use probability-ordering with $CL=0.95$.
	
	The goal is to solve:
	
	\[
	\int_{\N(x;\mu,1)\geq q(CL)} \N(x;\mu,1) \dd x = 0.95
	\]
	
	This is equivalent to asking which $x$ values (ordered centrally, which corresponds to probability ordering through symmetry in this case) do we need to integrate on to achieve a $95\%$ integral of a unitary variance Normal distribution? Luckily, as we can imagine, this is independent of $\mu$, and the Normal distribution tables\todo{ref and more precise than 1.96 pls} come to our rescue, as we can see that the interval in $x$ is:
	
	\[
	x\in\left[\mu-1.96,\mu+1.96\right]
	\]
...
These belts are of great didactic caliber, and are useful in visualizing what it is that we are actually doing under this whole mathematical abstraction. We will never really draw the belts out and explicitly intersect them to find intervals, rather we are visualizing for all $x$ the dependence of the interval on the measured data. By solving equation \ref{eq:int_definition} (numerically, or if we are lucky, analytically), we are actually finding the bounding functions of these intervals. \todo{This could be a good place for the code}


\begin{example}
	Let's draw a Neyman belt for a case that can solved analytically. We will study the common example of an observable $x$ distributed following a uniform \textit{pdf} $p(x;m) = \frac{1}{m}$ for $0\leq x \leq m$ and $0$ outside of that support. We observe one $x$ and wish to build an interval estimate for $m$ with a $90\%$ \textit{CL}. 
