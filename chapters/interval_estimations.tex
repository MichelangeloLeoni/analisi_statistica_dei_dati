...
From the point of view of Neyman's construction of confidence intervals, all three methods can be understood as different ways of defining the same fundamental object: a region of acceptance $\mathcal{P}(\mu)$ in the space of possible observations $n$, such that
\[
P(n \in \mathcal{P}(\mu) \mid \mu) = CL
\]
...
\[
\text{CI}(n_{\text{obs}}) = \{ \mu: n_{\text{obs}} \in \mathcal{P}(\mu) \}
\]
...
\[
\text{(quantile condition in } \mu \text{)} \quad \text{or} \quad \lambda(n_{\text{obs}}, \mu) = \text{constant}
\]
to determine the set of values of $\mu$ compatible with the observation. In these cases, there is no need to explicitly construct the full acceptance region $\mathcal{P}(\mu)$ for every $\mu$ in advance; instead, the interval is obtained by inverting a condition evaluated at the observed $n_{\text{obs}}$.
...
\[
R(n,\mu) = \frac{P(n;\mu)}{P(n;\hat\mu_n)}
\]
For this fixed $\mu$, the values of $n$ are sorted according to decreasing $R(n;\mu)$, and then included in the acceptance region $\mathcal{P}(\mu)$ until the desired total probability is reached:
\[
\sum_{n \in \mathcal{A}(\mu)} P(n;\mu) = CL
\]

Repeating this procedure for all values of $\mu$ produces a complete mapping:
\[
\mu \;\longrightarrow\; \mathcal{P}(\mu)
\]
